...
#begin try block for currency conversion
    try:
        ...
        from .load_cur import CURRENCY_VAL 
    ...
    df['margins_plit']=1
    df['candidate']=df['Contractor name']
    df['employee']='Unassigned'
    df['tid']=df['timesheet_key']
    df['split_employee']='Unassigned'
    ...
    df.rename(columns={"Date": "date", "Unnamed: 1": "job", "CompanyName": "company_name", "EntryQuantity":"entry_quantity","Units":"units", "PayRate":"pay_rate", "ChargeRate":"charge_rate",
                    "MarginSplitConsultantFullName":"employee", "MarginSplitPercent":"margins_plit", "Job description": "job_description"
                    },  inplace = True)
    print(df.head())
